...
const TargetCursor = ({ targetSelector = '.cursor-target', spinDuration = 2, hideDefaultCursor = true }) => {
    ...
    useEffect(() => {
        ...
        // Add style element to hide cursor only on target elements
        const styleId = 'target-cursor-style';
        let styleElement = document.getElementById(styleId);

        if (!styleElement && hideDefaultCursor) {
            styleElement = document.createElement('style');
            styleElement.id = styleId;
            styleElement.textContent = `
                ${targetSelector} { cursor: none !important; }
                ${targetSelector} * { cursor: none !important; }
            `;
        ...
        const enterHandler = e => {
            ...
            showCursor();  // Show cursor when entering target
            ...
            const leaveHandler = () => {
                activeTarget = null;
                isAnimatingToTarget = false;
                hideCursor();  // Hide cursor when leaving target
    ...
    }, [targetSelector, spinDuration, moveCursor, constants, hideDefaultCursor, showCursor, hideCursor]);
